...
### Plotting Age Intervals - Slide 10
```{r}
crime %>%
  filter(vict_age > 14) %>%
...
crime %>%
  ...
  summarise(crime_count = n())
```
## Plotting the Total Crime Incidents Over the Years - Slide 11
```{r}
crime %>%
  group_by(year_occurred) %>%
  summarise(crime_count = n()) %>%
...
##Plotting the Pie Chart - Slide 12
```{r}
...
## Plotting the Top 10 Most Used Weaponry - Slide 13
```{r}
crime %>%
...
## Most Common Month Crimes Occur - Slide 14
```{r, fig.width=10, fig.height=6}
